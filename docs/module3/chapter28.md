...
# Chapter 28: Deployment to Jetson
...
- Understand key principles of Deployment to Jetson
...
class Chapter28Example(Node):
    def __init__(self):
        super().__init__('chapter_28_example')
        self.get_logger().info('Chapter 28: Deployment to Jetson')
        # Implementation details...

def main():
    rclpy.init()
    node = Chapter28Example()
    rclpy.spin(node)
