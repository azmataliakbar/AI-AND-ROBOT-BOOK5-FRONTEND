...
- Understand key principles of ROS 2 Core Concepts
...
class Chapter2Example(Node):
    def __init__(self):
        super().__init__('chapter_2_example')
        self.get_logger().info('Chapter 2: ROS 2 Core Concepts')
        # Implementation details...

def main():
    rclpy.init()
    node = Chapter2Example()
    rclpy.spin(node)
